...
// OpenWithDB opens a store suing an existing bun.DB and specificed config options.
...
// (note: there is not need to run this after invoking Store.Init().)
...
// Signature valudation of signed CoRIMs is not supported. If insecure
...
// GetManifest returns the manifiest associated with the specified manifest ID.
...
// DeteleManifest deletes the manifiest associated with the specified manifest ID,
...
// GetValueTriples returns a slice of ValueTriple's whos environment matches
...
// GetKeyTriples returns a slice of KeyTriple's whos environment matches
// the one provided. If exact is true, any unset fields in the provided
// environment must be NULL in the database; otherwise, unset fields will
// match any value.
